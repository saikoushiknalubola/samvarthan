    ...
    <section className={["w-full max-w-full", className].filter(Boolean).join(" ")}>
      <div className="flex items-center justify-between gap-3 mb-4">
        <div className="min-w-0">
          <h2 className="text-xl sm:text-2xl font-semibold tracking-tight">Reports & Export</h2>
          <p className="text-sm text-muted-foreground mt-0.5">Build professional reports with Samvartana branding and export to PDF/CSV.</p>
        </div>
      ...
      <Tabs defaultValue="builder" className="w-full">
              ...
              <CardContent className="space-y-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-between rounded-lg border p-3 bg-secondary">
                      <div className="space-y-1">
                        <p className="text-sm font-medium">Include Samvartana logo</p>
                        <p className="text-xs text-muted-foreground">Adds a branded header to all pages</p>
                      </div>
                      <Switch
                    ...
                    <div className="flex items-center justify-between rounded-lg border p-3 bg-secondary">
                      <div className="space-y-1">
                        ...
                        <p className="text-xs text-muted-foreground">Insert a styled cover with summary</p>
                      </div>
                      <Switch
                    ...
                    <div className="space-y-2">
                      ...
                      >
                        <SelectTrigger id="theme" className="bg-card">
                ...
                <div className="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
                  <div className="flex items-center gap-2">
                    <Button
                      ...
                      className="bg-[var(--color-primary)] text-[var(--color-primary-foreground)] hover:opacity-95"
                  ...
                  <div className="flex items-center gap-3">
                    <Select value={sharePermission} onValueChange={(v: Permission) => setSharePermission(v)}>
                      <SelectTrigger className="w-[160px] bg-card" aria-label="Share permission">
                ...
                {shareLink && (
                  <div className="rounded-lg border p-3 bg-accent/40">
                    <p className="text-xs text-muted-foreground mb-1">Share link (expires in 7 days)</p>
                    <div className="flex items-center gap-2">
                      <Input readOnly value={shareLink} className="bg-card" />
                      <Button
                        variant="secondary"
                        className="bg-secondary text-foreground hover:bg-muted"
                        onClick={() => {
        ...
        <TabsContent value="preview" className="mt-4">
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-base">Full-page preview</CardTitle>
            </CardHeader>
            <CardContent>
        ...
        <TabsContent value="library" className="mt-4">
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-base">Reports library</CardTitle>
            </CardHeader>
            <CardContent>
                      ...
                      <TableRow key={r.id} className="hover:bg-secondary">
                        ...
                        <TableCell>
                          <div className="inline-flex items-center gap-2">
                            {r.template === "executive" && <FileChartPie className="size-4 text-[var(--color-primary)]" />}
                            {r.template === "technical" && <FileChartColumn className="size-4 text-[var(--color-primary)]" />}
                            {r.template === "compliance" && <FileDigit className="size-4 text-[var(--color-primary)]" />}
                            <span className="text-sm">{TEMPLATE_LABEL[r.template]}</span>
                          </div>
                        </TableCell>
                        ...
                        <TableCell className="min-w-0">
                          <div className="flex flex-wrap gap-1">
                            {r.tags.map((t) => (
                              <Badge key={t} variant="secondary" className="bg-accent text-[var(--color-accent-foreground)]">
                                {t}
                              </Badge>
                            ))}
                          </div>
...
function SectionToggle({
  ...
  return (
    <div className="flex items-center justify-between gap-4 rounded-md border p-3 bg-secondary">
      <div className="min-w-0">
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between gap-3 pb-3 border-b">
        <div className="min-w-0">
          <h3 className={["text-base sm:text-lg font-semibold leading-tight", theme.text].join(" ")}>
            {title}
          </h3>
        ...
        {branding.includeLogo && (
          <div className="shrink-0">
            <div className="flex items-center gap-2">
              <div className="size-8 rounded-lg bg-[var(--color-primary)] text-[var(--color-primary-foreground)] grid place-items-center text-xs font-bold">
                SV
              </div>
              <span className={["text-sm font-semibold hidden sm:inline", theme.accent].join(" ")}>Samvartana</span>
            </div>
          </div>
        )}
      </div>
      ...
      {branding.coverPage && (
        <div className="mt-4 rounded-lg border bg-secondary p-4">
          <p className="text-sm font-medium">Cover summary</p>
          <p className="text-xs text-muted-foreground">
      ...
      {/* Summary */}
      {sections.summary && (
        <SectionBlock icon={<FileChartPie className="size-4" />} title="Executive Summary">
          <p className="text-sm">
      ...
      {sections.methodology && (
        <SectionBlock icon={<TableOfContents className="size-4" />} title="Methodology">
      ...
      {/* Results */}
      {sections.results && (
        <SectionBlock icon={<FileChartLine className="size-4" />} title="Key Results">
      ...
      {/* Charts */}
      {sections.charts && (
        <SectionBlock icon={<ChartBar className="size-4" />} title="Charts & Visualizations">
      ...
      {/* Tables */}
      {sections.tables && (
        <SectionBlock icon={<FileChartColumn className="size-4" />} title="Data Table">
            ...
            <Table>
              ...
              <TableBody>
                <TableRow>
                  ...
                  <TableCell>tCO₂e</TableCell>
                  <TableCell className="text-right">12,430</TableCell>
                  <TableCell className="text-right">13,575</TableCell>
                </TableRow>
                <TableRow>
                  <TableCell>Energy</TableCell>
                  <TableCell>MWh</TableCell>
                  <TableCell className="text-right">28,940</TableCell>
                  <TableCell className="text-right">29,880</TableCell>
                </TableRow>
                <TableRow>
                  <TableCell>Water</TableCell>
                  <TableCell>ML</TableCell>
                  <TableCell className="text-right">4.12</TableCell>
                  <TableCell className="text-right">4.07</TableCell>
                </TableRow>
              </TableBody>
              <TableCaption className="text-xs text-muted-foreground">Table generated with Samvartana styling</TableCaption>
            </Table>
          </div>
        </SectionBlock>
      )}
      ...
      {sections.recommendations && (
        <SectionBlock icon={<SwatchBook className="size-4" />} title="Recommendations">
      ...
      {/* Appendix */}
      {sections.appendix && (
        <SectionBlock icon={<TableOfContents className="size-4" />} title="Appendix">
          <p className="text-sm text-muted-foreground">Additional references, data dictionaries, and calculation factors.</p>
        </SectionBlock>
      )}
    </div>
  )
          ...
          return (
            <div key={i} className="w-full">
              <div className="flex items-center justify-between">
                <span className="text-xs text-muted-foreground">{labels[i] ?? `Series ${i + 1}`}</span>
                <span className="text-xs">{v}%</span>
              </div>
              <div className="h-2 rounded bg-muted overflow-hidden">
                <div className={["h-2 rounded", theme[i % theme.length]].join(" ")} style={{ width: `${width}%` }} />
              </div>
            </div>
          )
        })}
